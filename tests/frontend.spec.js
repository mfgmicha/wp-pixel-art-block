...
test.describe( 'Frontend Pixel Art Block', () => {
	test.beforeEach( async ( { page } ) => {
		...
		await page.waitForSelector( '.wp-block-mfgmicha-pixel-art-creator', {
			timeout: 10000,
		} );
	} );
	...
	test.describe( 'Block Container', () => {
		test( 'block wrapper element exists with correct classes', async ( {
			page,
		} ) => {
	...
	test.describe( 'Color Palette', () => {
		...
		test( 'color swatches are rendered from theme palette', async ( {
			page,
		} ) => {
			const swatches = page.locator(
				'.pixel-art-creator-palette__swatch'
			);
		...
		test( 'swatches have background colors set', async ( { page } ) => {
			const firstSwatch = page
				.locator( '.pixel-art-creator-palette__swatch' )
				.first();
			const bgColor = await firstSwatch.evaluate(
	...
	test.describe( 'Grid', () => {
		...
		test( 'grid has cells based on configured columns/rows', async ( {
			page,
		} ) => {
		...
		test( 'cells have proper accessibility attributes', async ( {
			page,
		} ) => {
			const firstCell = page
				.locator( '.pixel-art-creator-grid__cell' )
				.first();
			await expect( firstCell ).toHaveAttribute( 'role', 'button' );
			await expect( firstCell ).toHaveAttribute( 'aria-label' );
		} );

		test( 'clicking a cell paints it with active color', async ( {
			page,
		} ) => {
		...
		test( 'drag to paint paints multiple cells', async ( { page } ) => {
			...
			const firstCellColor =
				await firstCell.getAttribute( 'data-cell-color' );
			const secondCellColor =
				await secondCell.getAttribute( 'data-cell-color' );

			expect( firstCellColor ).toBeTruthy();
			expect( secondCellColor ).toBeTruthy();
		} );

		test( 'keyboard accessibility - Enter key paints cell', async ( {
			page,
		} ) => {
			const firstCell = page
				.locator( '.pixel-art-creator-grid__cell' )
		...
		test( 'keyboard accessibility - Space key paints cell', async ( {
			page,
		} ) => {
			const secondCell = page
				.locator( '.pixel-art-creator-grid__cell' )
		...
		test( 'cell can be repainted with different color', async ( {
			page,
		} ) => {
			const cell = page
				.locator( '.pixel-art-creator-grid__cell' )
	...
	test.describe( 'Reset Button', () => {
		...
		test( 'clicking reset clears all painted cells', async ( { page } ) => {
			...
			let cellColor = await cells
				.nth( 0 )
				.getAttribute( 'data-cell-color' );
			expect( cellColor ).toBeTruthy();
	...
	test.describe( 'LocalStorage Persistence', () => {
		...
		test( 'grid state persists after page reload', async ( { page } ) => {
			...
			const firstSwatchColor = await swatches
				.first()
				.evaluate( ( el ) => el.getAttribute( 'data-swatch-color' ) );
	...
	test.describe( 'Integration', () => {
		test( 'full workflow: select color, paint cells, change color, paint more, reset', async ( {
			page,
		} ) => {
			const swatches = page.locator(
				'.pixel-art-creator-palette__swatch'
